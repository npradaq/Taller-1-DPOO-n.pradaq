...
public class CalculadoraEstadisticas
{
	...
	public String paisPorNombreAtleta(String nombreAtletaConsulta) {
		
		boolean flag = true;
		int index = 0;
		String paisAtleta = "";

		while(flag && index < atletas.size()){
			Atleta atletaTemp = atletas.get(index);

			if(nombreAtletaConsulta.equals(atletaTemp.darNombre()) ){
				paisAtleta = atletaTemp.darPais().darNombre();
				flag = false;
			}
			else {
				index++;
